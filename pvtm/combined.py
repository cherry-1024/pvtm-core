...
timeline_rwe = pd.read_csv(args['input']+"/comparison_results_JSE_RWE/combined_corpus/timeline_overview_2.csv",index_col='Unnamed: 0')

# load data for plots
bhtsne_3d = pd.read_csv(args['input'] + '/JSE_RWE/bhtsne_3d.csv', names=['x', 'y', 'z'])
bhtsne_2d = pd.read_csv(args['input'] + '/JSE_RWE/bhtsne_2d.csv', names=['x', 'y'])
...
layout= html.Div(children=[
    ...
    html.Div([
        html.Div([
            ...
            html.Img(id='img_3', style={'width': '500px'})
    ...
    html.Div([
        ...
        dcc.Tabs(
            ...
            value=2,
            id='tabs-scatter_3'),
        html.Div(id='scatter-output_3')
    ]),
    html.Div(id='combined-content')
])

@app.callback(Output(component_id='img_3', component_property='src'),
              [Input(component_id='input-value_3', component_property='value')]
              )
def update_img(value):
    try:
        image_filename = args['input'] + '/JSE_RWE/wordclouds/topic_{}.pdf.png'.format(
            value)
...
@app.callback(Output(component_id='timeline_3', component_property='figure'),
              [Input(component_id='input-value_3', component_property='value')]
              )
...
@app.callback(Output('scatter-output_3', 'children'),
              [Input('tabs-scatter_3', 'value')])
